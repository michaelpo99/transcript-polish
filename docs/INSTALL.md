...
- 安裝程式目前會將 `setuptools` 維持在 `<82`，避免與 `torch 2.12.0` 的已知相依限制衝突。
- 版本不符合時才安裝或更新。
...
安裝程序結束前，無論本次有沒有變更 mode，甚至前面同步套件時發生錯誤，都會顯示目前預設 mode 與設定檔位置，方便之後手動修改。

若日後使用者設定檔仍寫著 `quality`，但環境已缺少量化套件或 CUDA，CLI 也會自動退回 `standard`。
